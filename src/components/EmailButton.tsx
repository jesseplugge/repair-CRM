...
export function EmailButton({ id, action, defaultEmail, label }: { id: string; action: EmailAction; defaultEmail?: string | null; label?: string }) {
  ...
  function send() {
    setError(null);
    const email = defaultEmail || window.prompt(t('promptEmail'));
    if (!email) return;
  ...
  return (
    <div>
      ...
      >
        ...
        {pending ? t('busy') : sent ? t('sent') : label ?? t('default')}
      </button>
